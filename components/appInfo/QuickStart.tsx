         ...
         >
            ...
            <View className="mb-6">
               <View className="px-6 mb-3 flex-row justify-between items-center">
                  <Text className="text-xs font-bold uppercase tracking-widest text-slate-400">See it in action</Text>
               </View>
                     ...
                     return (
                        <Pressable
                           ...
                           className={`flex-row items-center justify-center rounded-full px-4 py-2 border ${
                              isActive ? 'bg-slate-900 border-slate-900 dark:bg-white dark:border-white' : 'bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-800'
                           }`}
                        >
                           {isActive && <RefreshCw size={12} color={isActive ? '#ffffff' : '#94a3b8'} style={{ marginRight: 8 }} />}
                           <Text className={`text-xs font-bold text-center ${isActive ? 'text-white dark:text-black' : 'text-slate-600 dark:text-slate-400'}`}>
                              {scenario.label}
                           </Text>
                        </Pressable>
                     );
                        ...
                        <View key={step.key}>
                           {isPivotPoint && (
                              <TimelinePivot variant="default">
                                  ...
                                  <Text className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
                                    You can continue on your own, or get an optional AI
                                    assisted analysis based on your{' '}
                                  ...
                                  <View className="mt-3 flex-row gap-8">
                                    <View className="flex-1">
                                        ...
                                        <Text className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                            Continue with the{' '}
                                            <Text className="font-bold">standard questions</Text>{' '}
                                            for <Text className="font-bold text-dispute-text dark:text-dispute-textDark">Dispute (D)</Text>{' '}
                                            and <Text className="font-bold text-energy-text dark:text-energy-textDark">Energy (E)</Text>.
                                        </Text>
                                    </View>
                                    ...
                                    <View className="flex-1">
                                        ...
                                        <Text className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                            Get an <Text className="font-bold">analysis</Text> of
                                            your entry and{' '}
                                            <Text className="font-bold">tailored questions</Text>{' '}
                           ...
                           <TimelineItem step={step} variant="default">
                              <View className="mt-3 overflow-hidden rounded-lg bg-slate-50 dark:bg-black/20 px-3 py-2.5">
                                 <Text className="mb-1 text-[10px] font-bold uppercase tracking-wider text-slate-400/80">Example</Text>
                                 <Text className="font-medium italic text-slate-700 dark:text-slate-300">{exampleText}</Text>
                              </View>
                           </TimelineItem>
                        </View>
                     );
