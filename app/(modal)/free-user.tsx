...
export default function FreeUserChoiceScreen() {
   ...
   useEffect(() => {
      const id = requestAnimationFrame(() => modalRef.current?.present());
      return () => cancelAnimationFrame(id);
   }, []);

   // 2. GUARDED DISMISSAL
   const handleDismiss = useCallback(() => {
      // If we are moving to Login, do NOT fire router.back()
   ...
   const handleChoice = (requiresAuth: boolean) => {
      ...
      const path = requiresAuth ? `/dispute/${entryId}?analyze=1` : `/dispute/${entryId}`;
   ...
   return (
      ...
      >
         <BottomSheetView style={{ paddingHorizontal: 24, paddingTop: 16, paddingBottom: insets.bottom + 20 }}>
            {/* Header */}
            <View className="mb-6">
               <Text className="text-sm font-semibold text-amber-600 dark:text-amber-400 mb-1">Free plan</Text>
               <Text className="text-2xl font-bold mb-1" style={{ color: theme.text }}>How do you want to dispute?</Text>
               <Text className="text-base" style={{ color: theme.subText }}>Choose AI analysis or jump into the guided steps.</Text>
            </View>
            ...
            <View className="gap-3">
               ...
               >
                  <View className="flex-1 pr-3">
                     <Text className="text-lg font-semibold" style={{ color: theme.amberText }}>Let AI analyze this</Text>
                     <Text className="text-sm mt-1" style={{ color: theme.amberSub }}>Get an instant breakdown and suggestions.</Text>
                  </View>
               ...
               >
                  <View className="flex-1 pr-3">
                     <Text className="text-lg font-semibold" style={{ color: theme.text }}>Go to dispute steps</Text>
                     <Text className="text-sm mt-1" style={{ color: theme.subText }}>Work through the guided prompts without AI.</Text>
                  </View>
